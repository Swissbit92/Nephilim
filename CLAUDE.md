...
- **Sampling (2026-08-23):** persona `model_preferences` now forwards `top_k`, `top_p` and `repeat_last_n` in addition to `temperature`/`min_p`/`repeat_penalty` — the first three were declared on `SamplingPreset` and silently dropped. Both spellings of the penalty (`repeat_penalty`, `repetition_penalty`) are accepted; only the former was read while the schema documented the latter. `repeat_last_n` matters most: Ollama's default is **64 tokens (~50 words)** — enough to stop a sentence repeating inside one reply, far too short to notice a repeated paragraph. Out-of-range values are omitted, never clamped, so the caller's default applies.
- **⚠️ `repeat_last_n` must be ≥ 0 — `-1` is a HARD 400 (2026-09-22).** It used to mean "the whole context"; llama.cpp removed that in PR #26524 (2026-08-04) and Ollama forwards the value verbatim, so `{"repeat_last_n":-1}` returns `HTTP 400 Field 'repeat_last_n': Value must be between 0 <= value <= 2147483647`. On the greet path `_complete_or_503` re-raises that as `503 "LLM service temporarily unavailable"`, so it does **not** look like a sampler fault. `get_persona_sampling_overrides` is the guard that binds — cards load via `load_persona_card_lenient`, which warns and returns the raw dict regardless, so the `ge=0` schema bound documents the contract but cannot enforce it. Also: `typical_p` is retired and 400s (`"typical_p is no longer supported"`) — never emit it. And **`repeat_penalty` is applied to PROMPT tokens too, flat, with no positional decay**, so a large window penalises the persona's own pinned voice exemplars for the whole session; keep the penalty light (gwen: 1.05 / 384, both chosen rather than derived).
- **⚠️ Ollama's `repeat_penalty` default became 1.0 — i.e. OFF — in commit `6a261db7` (2026-08-12).** A persona that does not declare one runs with no repetition penalty at all. Measured on this box: `repeat_last_n = 64, repeat_penalty = 1.000, dry_multiplier = 0.000`. DRY and XTC are absent from Ollama's `Options` entirely and are **accepted and silently ignored** (HTTP 200 + `level=WARN source=types.go:1048 "invalid option provided"`) — that WARN line is the only way to tell "sent" from "applied", and `test_sampler_wire_arrival.py` exists to assert exactly that.
- **`min_p` is path-dependent, not globally blocked (corrected 2026-09-22).** On the **legacy** path it needs `OLLAMA_COMPLETION_BACKEND=http` (default `langchain`): `langchain_ollama.OllamaLLM` coerces options through `ollama.Options`, which has **no `min_p` field** — verified two ways against the installed 1.0.1 (upstream langchain-ai/langchain#32744), so no config change reaches it and `services/ollama_http.py` posts to `/api/generate` with the options dict verbatim. Defaults to `langchain` for one soak cycle because the HTTP path is new wire code; a requested `min_p` logs a warning rather than vanishing. **But the tool brain passes a plain dict to `Client.chat` and `ChatRequest.options` accepts a `Mapping` without coercing — so on the tool-brain path (which is every gwen turn) `min_p` DOES reach the wire.** Verified live.
- **The tool brain's prose call uses the persona's samplers; a tool DECISION keeps temp 0.4 (2026-09-22).** ADR-008 resolved to a single model and TB6 made the tool brain's own answer user-facing on chitchat, but its `opts` were hardcoded `{"temperature": 0.4}` with no persona overrides — so the card's settings never reached the text the user reads, and `c00cf084` ("widen the repetition-penalty window") was inert because it was verified through `OllamaLLM`, the *legacy* transport. `routes/chat.py` now resolves `get_persona_sampling_overrides(card)` and passes it with `prose_expected`; persona voice is deliberately kept out of tool-argument JSON. **Resolve overrides in the ROUTE, never inside the service** — `get_persona_sampling_overrides` reads the module-level settings singleton frozen at import, so a deeper call is invisible to any test that sets the env and clears the cache. Tool-brain calls also send `keep_alive` now: Ollama applies it per request, last-one-wins, so omitting it silently reverted `OLLAMA_KEEP_ALIVE=-1` every turn. **Exactly one generation per turn is load-bearing** (TB6 costed the alternative at ~16 tok/s) — do not "fix" this by deciding cold then regenerating warm.
